...
export default function FoOffice() {
  ...
  useEffect(() => {
    const fetchNOCs = async () => {
      setLoading(true);
      try {
        const queryString = yearFilter ? `?year=${yearFilter}&stage=FO_OFFICE` : "?stage=FO_OFFICE";
        ...
        const formatted = data.map((noc) => {
          const dswApproval = noc.approvals?.find((a) => a.stage === "FO_OFFICE");
          return {
            id: noc._id,
