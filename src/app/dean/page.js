...
export default function DeanNocs() {
  ...
  useEffect(() => {
    const fetchNOCs = async () => {
      setLoading(true);
      try {
        const queryString = yearFilter ? `?year=${yearFilter}&stage=DEAN` : "?stage=DEAN";
        ...
        const formatted = data.map((noc) => {
          const dswApproval = noc.approvals?.find((a) => a.stage === "DEAN");
          return {
            id: noc._id,
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-8">
      ...
      <div className="text-center mb-8">
        <Typography variant="h4" className="font-bold text-indigo-700 mb-2">
          NOC Requests for DEAN Approval
        </Typography>
